...
namespace CRM.Web.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        {
            // Traduction basique des erreurs courantes d'Identity
            var translations = new Dictionary<string, string>
            {
                {"Passwords must have at least one non alphanumeric character.", "Les mots de passe doivent contenir au moins un caractère non alphanumérique."},
                {"Passwords must have at least one digit ('0'-'9').", "Les mots de passe doivent contenir au moins un chiffre ('0'-'9')."},
                {"Passwords must have at least one uppercase ('A'-'Z').", "Les mots de passe doivent contenir au moins une lettre majuscule ('A'-'Z')."},
                {"User name is already taken.", "Ce nom d'utilisateur est déjà pris."},
                {"Email is already taken.", "Cet email est déjà utilisé."}
            };

            return translations.ContainsKey(error) ? translations[error] : error;
        }
    }
}
